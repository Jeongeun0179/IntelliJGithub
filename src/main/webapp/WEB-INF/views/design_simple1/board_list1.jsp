...
<%
	...
	if (request.getParameter("coffee_id") != null) {
		...
		if ( result == 1 ) {
			//Cart cartList = (Cart) request.getAttribute("cartList");
			 // 카트 데이터 가져오기
			//System.out.println(cartList.toString());

			//int quantity = cartList.getQuantity();
			//int coffeeId = cartList.getCoffee_id();

			//if (cartList.getCoffee() != null && !cartList.getCoffee().isEmpty()) {
		...
		// cart 테이블에 데이터가 하나라도 있는지 확인
//		if (cartSelectAll != null && !cartSelectAll.isEmpty()) {
//			for ( Cart cart : cartSelectAll) {
//				for (Coffee coffee : cart.getCoffee()) {
//					int quantity = cart.getQuantity();  // Cart의 수량
//					int coffeePrice = coffee.getCoffee_price();  // 커피의 가격
//
//					// 커피 가격과 수량을 기반으로 가격 계산 예시
//					totalPrice += quantity * coffeePrice;
//
//					// 필요한 추가 로직 작성 가능
//					//System.out.println("단가: " + coffeePrice);
